...
def execute_all(offset=0):
    ...
    code_path = input_folder / 'gpt_answers_func_manual'
    input_path = input_folder / 'gpt_more_inputs_20_3.5_fix'

    output_path = input_folder / 'gpt_more_outputs_20_3.5_fix'
    output_path.mkdir(parents=True, exist_ok=True)

    data = read_dataset(input_folder / 'raw_datasets.jsonl')
    ...
    with open(output_path / 'all.jsonl', 'w') as wf:
        for given_qid in tqdm(sorted(data.keys())):
            ...
            if given_qid in SKIP_IDS:
                ...
                continue
            in_file = input_path / '{}.txt'.format(given_qid)
...
def execute_all_fix_mode(offset=0):

    input_folder = Path('./datasets/TDE/')
    code_path = input_folder / 'gpt_answers_func_manual'
    previous_file = input_folder / 'gpt_more_outputs_20_3.5' / 'all.jsonl'
    previous_objs = defaultdict(list)
    for item in load_data(previous_file):
        previous_objs[item['oq_id']].append(item)
    ...
    with open(output_path / 'all.jsonl', 'w') as wf:
        for given_qid in tqdm(sorted(data.keys())):
            ...
            if given_qid in SKIP_IDS:
                logging.info('ignoring id: %s', given_qid)
                continue
            if given_qid not in REVISED_MORE_INPUT_IDS:
                logging.info('using old entry for id: %s', given_qid)
                for item in previous_objs[given_qid]:
...
if __name__ == '__main__':
    ...
    # execute_all()
    execute_all_fix_mode()
    
    # output = read_inputs(Path('./datasets/TDE/gpt_more_inputs_3.5') / '1.txt')
    # output = read_inputs(Path('./datasets/TDE/gpt_more_inputs_3.5') / '5.txt')
    # output = read_inputs(Path('./datasets/TDE/gpt_more_inputs_3.5') / '7.txt')
    # print(output)