...
def build_text_multichoice_question(pdf_name, dest_path):
    ...
    with open(dest_path, 'w') as wf:
        for _, _row in test_pdf.iterrows():
            ...
            choices.append('{}. {}'.format('({})'.format(len(gts)+1), 'None of the above'))

            question = '\n'.join(choices)
            answers = ['({})'.format(gt_index+1)]
            wf.write('{}\n'.format(json.dumps({
                'question_id': _id,
...
if __name__ == '__main__':
    ...
    # build_text_multichoice_question('testrun_1_sgts', './datasets/STD/dataset/run1_test_desc.jsonl')
    generate_prompt(
        './datasets/STD/dataset/run1_test_desc.jsonl', 
        './datasets/STD/dataset/run1_test_desc_test.jsonl',
        './prompts/std_desc_2_desc.txt'
        )
